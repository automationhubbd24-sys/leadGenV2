...
async function runCampaign(jobId: string, leads: any[], smtps: any[]) {
  ...
  try {
    for (let i = 0; i < leads.length; i++) {
      ...
      const finalHtml = cleanBody.includes('<br>') || cleanBody.includes('<p>') 
        ? cleanBody 
      ...
      // Increased delay to be more natural (20 to 40 seconds)
      const baseDelay = activeSmtps.length > 1 ? 15000 : 25000;
